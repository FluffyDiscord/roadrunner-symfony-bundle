 ...
 * JobRoutingListener rehydrates it through the Native serializer, and the #[AsJobHandler] runs with
 * a real object carrying the exact token (observable via /app/var/handled.json). A separate raw,
 * non-enveloped task pushed straight via the RR Jobs API still reaches a plain JobsRunEvent listener
 * — proving the bus is purely additive.
 */
#[Group('jobs-live')]
class JobBusLiveTest extends AbstractJobsLiveTestCase
{
    public function testDispatchedMessageIsRoutedToHandler(): void
    {
        ...
        self::assertNotNull($handled, 'the #[AsJobHandler] never wrote its proof');
